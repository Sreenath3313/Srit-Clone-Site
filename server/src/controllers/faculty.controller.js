...
export async function getAssignedClasses(req, res) {
  try {
    ...
    // Log sample entry in development only
    if (data && data.length > 0 && process.env.NODE_ENV !== 'production') {
      console.log(`[Faculty] Sample timetable entry:`, JSON.stringify(data[0], null, 2));
    }
...
export async function markAttendance(req, res) {
  try {
    ...
    // Use upsert to allow updating attendance for the same date
    // The UNIQUE constraint (student_id, subject_id, date) will handle conflicts
    // onConflict accepts a comma-separated string of column names (Supabase JS v2 syntax)
